...
{
    ui->setupUi(this);UlError erreur;
    ...
    erreur = laCarte.ulDConfigBit(BP3,DD_OUTPUT);
    if(erreur != ERR_NO_ERROR)
        qDebug() << "Config " << BP3 << " Code Erreur : " << erreur;
    erreur = laCarte.ulDConfigBit(BP4,DD_OUTPUT);
    if(erreur != ERR_NO_ERROR)
        qDebug() << "Config " << BP4 << " Code Erreur : " << erreur;

    connect(&timerBP,&QTimer::timeout,this,&::WidgetUSB202::on_TimerBP_timeOut);
           timerBP.start(500);
}
...
void WidgetUSB202::on_TimerBP_timeOut()
{
    quint8 valeur;
    UlError erreur;
